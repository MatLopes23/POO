...
CWindowSingleton::CWindowSingleton(const CWindowSingleton& h):x(h.x),y(h.y),cx(h.cx),cy(h.cy)
{
    my_canvas=new Canvas(h.Cget_tamanho(), h.Cget_tipo_fonte(), h.Cget_fonte(), h.Cget_pena(), h.Cget_pincel());
}

CWindowSingleton& CWindowSingleton::operator=(const CWindowSingleton& h)
{
    if (this==&h)
        return *this;
...
std::ostream& operator << (std::ostream& op, const CWindowSingleton& h)
{
...
std::istream& operator >> (std::istream& op, CWindowSingleton& h)
{
